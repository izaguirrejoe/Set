...
struct Grid
{
    ...
    private mutating func updateCellFrames(to cellSize: CGSize) {
        ...
        let adjustedCellSize = CGSize(width: cellSize.width * Constants.insetFactor, height: cellSize.height * Constants.insetFactor)

        if cellCount > 0 {
            for _ in 0..<cellCount {
               cellFrames.append(CGRect(origin: origin, size: adjustedCellSize))
...
extension Grid{
    private struct Constants{
        static let insetFactor: CGFloat = 9.0/10.0
    }
}
